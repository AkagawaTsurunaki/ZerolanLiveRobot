...
class FilterConfig(BaseModel):
    bad_words: List[str] = Field(default=[
        ...
        "作为一个人工智能",
        "作为一个AI"
    ], description=_("List of words to be filtered out in conversations."))
    strategy: FilterStrategyEnum = Field(FilterStrategyEnum.Default,
                                         description=_(
                                             "The filtering strategy to use, currently only supports 'default'. \n"
                                             "default: The word is filtered out as long as it matches."))


class ChatConfig(BaseModel):
    filter: FilterConfig = Field(FilterConfig(),
                                 description=_("Configuration for filtering in chat"))
    system_prompt: str = Field(
        ...
        description=_("System prompt used to guide the chatbot's behavior. \n"
    ...
    injected_history: List[str] = Field(default=[
        ...
        description=_("List of predefined messages to inject into the chat history. \n"
    ...
    max_history: int = Field(20,
                             description=_("Maximum number of messages to keep in chat history."))


class SpeechConfig(BaseModel):
    is_remote: bool = Field(default=False,
                            description=_(
                                "If this value is set to `True`, the system will assume that the TTS prompt files "
    ...
    prompts_dir: str = Field("resources/static/prompts/tts",
                             description=_("Directory path for TTS prompts. (Absolute path is recommended)\n"
    ...
    prompts: List[str] = Field(default=[],
                               description=_("If you set `is_remote` to `True`, you must config this!"))


class CharacterConfig(BaseModel):
    bot_name: str = Field("<YOUR_BOT_NAME>",
                          description=_("Name of the bot character."))
    chat: ChatConfig = Field(ChatConfig(),
                             description=_("Configuration for chat-related settings."))
    speech: SpeechConfig = Field(SpeechConfig(),
                                 description=_("Configuration for speech-related settings."))
